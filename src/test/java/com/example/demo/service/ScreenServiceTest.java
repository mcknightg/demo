...
class ScreenServiceTest {
  ...
  void before(){
    ...
    item1 = generator.nextObject(Screen.class);
    item2 = generator.nextObject(Screen.class);
    service = new ScreenService(this.repo);
  }

  @Test
  void shouldSave(){
    Mockito.when(repo.save(Mockito.any(Screen.class))).thenReturn(item1);
    Screen data = service.save(Screen.builder().build());
  ...
  void findAll(){
    ...
    List<Screen> all = StreamSupport.stream(service.findAll().spliterator(), false).collect(Collectors.toList());
  ...
  void search(){
    ...
    Page<Screen> page = new PageImpl<>(Arrays.asList(item1,item2));
    Mockito.when(repo.findAll(pageRequest)).thenReturn(page);
    List<Screen> all = new ArrayList<>(service.search(null, pageRequest).getContent());
    Assertions.assertEquals(2, all.size());
  }
